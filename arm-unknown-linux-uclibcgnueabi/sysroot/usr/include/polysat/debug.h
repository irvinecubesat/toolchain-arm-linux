 ...
 */
#define ERR_REPORT(lvl, fmt, ...) do { DBG_syserr(__LINE__, __FUNCTION__, \
                        __FILE__, (lvl), (fmt),## __VA_ARGS__); } while(0)
 ...
 */
#define ERRNO_EXIT(fmt, ...) do { if (errno) { ERR_REPORT(DBG_LEVEL_FATAL, \
 ...
 */
#define ERR_EXIT(cmd, fmt, ...) do { errno = 0; (cmd); ERRNO_EXIT((fmt), \
                                 ## __VA_ARGS__); } while(0)
 ...
 */
#define ERRNO_WARN(fmt, ...) do { if (errno) { ERR_REPORT(DBG_LEVEL_WARN, \
                           (fmt),## __VA_ARGS__); errno = 0; } } while(0)

/**
 * \brief This macro is the same as ERRNO_WARN but requires the actual command as a
 *  parameter so that errno can be reset before the function call.
 */
#define ERR_WARN(cmd, fmt, ...) do { errno = 0; (cmd); ERRNO_WARN((fmt), \
                                          ## __VA_ARGS__); }  while(0)
